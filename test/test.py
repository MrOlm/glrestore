...
def test_object_glacerized(BTO):
    ...
    assert glrestore.s3_utils.glacier_status(BTO.non_glacerized_file_loc, profile='sonn') == 'no-glacier'
    ...
    assert glrestore.s3_utils.glacier_status(BTO.restored_file_loc, profile='sonn') == 'glacier-restored'

    # Test ability to recognize *restoring* glacierized file
    cmd = "aws s3api restore-object --profile sonn --bucket sonn-current --key users/mattolm/testing_house/glrestore/N5_271_010G1_scaffold_min1000.fa --restore-request '{\"Days\":1,\"GlacierJobParameters\":{\"Tier\":\"Expedited\"}}'"
    subprocess.call(cmd, shell=True)
